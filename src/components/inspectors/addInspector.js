...
import { createInspector, cleanInspector } from '../../redux/inspectors/inspectorsCreate';
import { getCountries } from '../../redux/countries';
import { useTokenClaims } from '../public/hooks';


function AddInspector(props) {
    ...
    const { isLoading, hasError, inspector, isSuccess } = useSelector( state => state.inspector.create );
    ...
    React.useEffect( () => {
        if (!isLoading){
            if (isSuccess){
                setTimeout( () => history.push('/inspectors'), 1000 )
            }
        }
    }, [isLoading])
    ...
    const onFinish = (values) =>{
        ...
        dispatch(createInspector(values.CountryDivisionCode, values))
    }

    return (
        <div className="content mb-5">
            <div className="d-flex justify-content-between">
                <span className="square-indicator">مشخصات بازرس</span>
                <div className="ml-2">
                    <NavLinkRouter className="text-decoration-none" to="/inspectors">
            ...
            <div className="px-xl-5 mt-4">
                <Spin size="large" spinning={isLoading} >
                    ...
                    >
                        ...
                        <CountryDivisions form={form} visibleFields={activeCityFields} defaultCode={countryDivisionCode} isInspector={true} />
                        <Row>
                            <Col className="px-2 " xs={24} sm={12} md={12} lg={8} xl={8}>
                                <Form.Item 
                                    label="نام پدر"
