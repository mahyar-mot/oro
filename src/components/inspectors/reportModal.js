...
import { getInspectorsExport, getInspectorsPDFExport, cleanInspectorsFile } from '../../redux/inspectors/inspectorsFile';
...
const OverseerTableList = [
    ...
    {colName: "سطح فعالیت", colValue: "AssignmentType"},
    {colName: "توضیحات", colValue: "Description"},
    {colName: "تاریخ تولد", colValue: "BirthDate"},
    {colName: "وضعیت", colValue: "ProfileStatusInspector"},
]
...
const ReportModal = (props) => {
    ...
    const {inspectorExcelExport, isLoading} = useSelector(state => state.inspector.file)
    ...
    React.useEffect( () => {
        return () => dispatch(cleanInspectorsFile())
    }, [])

    React.useEffect( () => {
        if (inspectorExcelExport){
            if (!isLoading){
                setIsModalVisible(false)
            }
        }
    }, [inspectorExcelExport, isLoading])
    ...
    const handleOk = () => {
        ...
        dispatch(getInspectorsExport({countryDivisionCode, ...props.params, pageSize:listCount, columns: values}))
    };

    const handleOkPDF = () => {
        // setIsModalVisible(false);
        let values = form.getFieldValue("overseerCols")
        dispatch(getInspectorsPDFExport({countryDivisionCode, ...props.params, pageSize:listCount, columns: values}))
    };
