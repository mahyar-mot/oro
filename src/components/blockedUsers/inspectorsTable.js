...
import {getBlockedInspectors, cleanBlockedUsers, activateBlockedInspector} from '../../redux/blockedUsers';
...
const ConfirmModal = (props) => {
    ...
    return(
        <Modal 
            ...
            footer="" 
            title={<div className="text-center text-white" >تایید فعال سازی بازرس</div>} 
        ...
        >
            <div className="text-center p-4">
                <p >
                    آیا از فعال سازی بازرس مسدود شده مطمئن هستید؟
                </p>
                <div>
                    ...
                    <Button type="primary" loading={isLoading} onClick={() => dispatch(activateBlockedInspector(props.selectedUser.nationalNo))}>بله مطمئنم</Button>
                </div>
            </div>
        </Modal>
    )
...
function InspectorsListTable(props) {
    const {params = {}} = props;
    const {Column} = Table;
    ...
    React.useEffect(() => {
        ...
        if (countryDivisionCode){
            dispatch(getBlockedInspectors({countryDivisionCode, ...params}));
        }
                    ...
                    pagination={
                        {
                            ...
                            onChange: (page, pageSize) => {
                                dispatch(getBlockedInspectors({countryDivisionCode, ...params, page, pageSize}))
                                setCurrentPage(page)
            ...
            <Spin spinning={loadingResponsive}>
                <TableResponsive total={listCount}
                                 ...
                                 onChange={(page, pageSize) => {
                                     ...
                                     dispatch(getBlockedInspectors({countryDivisionCode, ...params, page, pageSize})).then(r => setLoadingResponsive(false))
                                     setCurrentPage(page)
                                     setCurrentPageSize(pageSize)
                                 }}
                >
...
export default InspectorsListTable
