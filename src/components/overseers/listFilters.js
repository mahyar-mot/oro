...
function ListFilters(props) {
    ...
    const {status, assignmentTitle, havingBranchType} = useSelector(state => state.basicInfo);
    // const [statusFilter, setStatusFilter] = React.useState(0);
    const [activeCityFields, setActiveCityFields] = React.useState(1);
    const [confirmedSearching, setConfirmedSearching] = React.useState(false)
    ...
    const onFieldsChange = (changedFields) => {
        ...
        if (changedFields.length) {
            ...
            }if (changedFields[0].name[0] === "Status" && changedFields[0].value === 5 ) {
                setConfirmedSearching(true)
            }else if (changedFields[0].name[0] === "Status"){
                setConfirmedSearching(false)
            }
        }
    }
    ...
    React.useEffect(() => {
        ...
        if (Object.keys(querys).length) {
            ...
            setTimeout(() => serParams(querys), 2500)
    ...
    return (
        <div className="content px-xl-5">
            ...
            >
                <CountryDivisions form={form} visibleFields={activeCityFields} InitialCDC={InitialCDC}/>
                ...
                <Row>
                    ...
                    <Col className="px-2" xs={24} sm={12} md={12} lg={8} xl={4}>
                        ...
                        >
                            <Select
                                ...
                                placeholder=" وضعیت ناظر را انتخاب کنید"
                        ...
                        confirmedSearching && (
                            <Col className="px-2" xs={24} sm={12} md={12} lg={8} xl={4}>
                                <Form.Item
                                    className=""
                                    label=" وضعیت تاییدی"
                                    name="confirmedFilterType"
                                >
                                    <Select
                                        size="large"
                                        showSearch
                                        dropdownClassName="text-right"
                                        placeholder=" وضعیت تایید را انتخاب کنید"
                                        // onChange={ value => setStatusFilter(value) }
                                    >
                                        <Select.Option value="1">همه</Select.Option>
                                        <Select.Option value="2">تایید شده</Select.Option>
                                        <Select.Option value="3">تایید شده مدیریتی</Select.Option>
                                    </Select>
                                </Form.Item>
                            </Col>
                        )
                    ...
                    <Col className="px-2" xs={24} sm={12} md={12} lg={8} xl={4}>
                        ...
                        >
                            <Select
                                ...
                                showSearch
                                dropdownClassName="text-right"
                                placeholder=" سطح فعالیت ناظر را انتخاب کنید"
                                // onChange={ value => setStatusFilter(value) }
                            >
                                {
                                    assignmentTitle.map((item, index) => (
                    ...
                    <Col className="px-2" xs={24} sm={12} md={12} lg={8} xl={4}>
                        <Form.Item
                            className=""
                            label=" وضعیت شعبه"
                            name="havingBranchType"
                        >
                            <Select
                                size="large"
                                showSearch
                                dropdownClassName="text-right"
                                placeholder=" وضعیت شعبه را انتخاب کنید"
                                // onChange={ value => setStatusFilter(value) }
                            >
                                {
                                    havingBranchType.map((item, index) => (
                                        <Select.Option value={String(item.value)} key={index}>{item.name}</Select.Option>
                                    ))
                                }
                            </Select>
