...
import { getRollcallList, cleanBranchesList} from '../../redux/branches/branchesList';
// import { getRollcallList, cleanBranchesList } from "../../redux/headOverseers/headOverseersList"
...
function ListTable(props) {
    ...
    React.useEffect(() => {
        ...
        if (nationalNumber){
            dispatch(getRollcallList(nationalNumber, {ownBranches: true, ...params}));
        }
                    ...
                    pagination={
                        {
                            ...
                            onChange: (page, pageSize) => {
                                dispatch(getRollcallList(nationalNumber, {ownBranches: true, ...params, page, pageSize}))
                                setCurrentPage(page)
                ...
                >
                    ...
                    <Column
                        ...
                        render={(text, record) => (
                            <>
                                {/* {
                                    record?.branchNumber && (<Button className="ml-3 px-1" type="secondary "  onClick={e => {
                                ...
                                <Button className="ml-3 mb-2" type="primary"  onClick={e => {
                                    history.push(UrlQuery(`/rollcalls/branch/${record.id}`, {
                                        ...params,
                                        page: currentPage,
                                        pageSize:currentPageSize
                                    }))
            ...
            <Spin spinning={loadingResponsive}>
                <TableResponsive total={listCount}
                                 ...
                                 onChange={(page, pageSize) => {
                                     ...
                                     dispatch(getRollcallList(nationalNumber, {ownBranches: true, ...params, page, pageSize})).then(r => setLoadingResponsive(false))
                                     setCurrentPage(page)
                                     setCurrentPageSize(pageSize)
                                 }}
                >
                            ...
                            return <table className="table table-striped border-0" key={i}>
                                ...
                                <RowResponsive title="عملیات "
                                               ...
                                               <>
                                                    {/* {
                                                        item?.branchNumber && (
                                                        <Button className="ml-3 px-1 border-warning" type="secondary " size="small" onClick={e => {
                                                    ...
                                                    <Button className="ml-3 mb-2" type="primary" size="small" onClick={e => {
                                                        history.push(UrlQuery(`/rollcalls/branch/${item.id}`, {
                                                            ...params,
                                                            page: currentPage,
                                                            pageSize:currentPageSize
                                                        }))
