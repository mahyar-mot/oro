                            ...
                            <div className="px-xl-5 mb-3">
                                <Row className="align-items-center">
                                    ...
                                    <Col xs={24} sm={24} md={24} lg={20} xl={21}>
                                        <Row>
                                            <Col className="px-2 mt-3" xs={24} sm={10} md={8} lg={10} xl={9}>

                                                <span className="link-color">  نام‌ونام‌خانوادگی : </span>
                                                <span>{ticket?.creatorFullName}</span>
                                            </Col>
                                            <Col className="px-2 mt-3" xs={24} sm={14} md={16} lg={14} xl={15}>
                                                <span className="link-color">  موقعیت جغرافیایی : </span>
                                                <span>{ticket?.countryDivisionDetail} </span>
                                            </Col>
                                            <Col className="px-2 mt-3" xs={24} sm={10} md={8} lg={10} xl={9}>
                                                <span className="link-color">  کدملی  : </span>
                                                <span>{ticket?.creatorNationalNo} </span>
                                            </Col>
                                            <Col className="px-2 mt-3" xs={24} sm={14} md={16} lg={14} xl={15}>
                                                <span className="link-color">  تلفن همراه  : </span>
                                                <span>{ticket?.creatorMobile} </span>
                                            </Col>
                                        </Row>
                                    </Col>
                                        ...
                                        Boolean(ticket?.tickets?.length) && ticket.tickets.map( (item, index) =>
                                            <Row key={index}>
                                                ...
                                                >
                                                    ...
                                                    <div className="text-wrapper p-3 mb-3" style={{borderRadius:`${ticket.creatorFullName === item.creatorFullName ? "7px 7px 0px 7px" : "7px 7px 7px 0px"}`}}>
                                                        <div className="d-flex justify-content-between mb-2">
                                                            <small className={classNames({"order-last": ticket.creatorFullName !== item.creatorFullName })}> <FaUserCircle color="gray" /> {item.creatorFullName}</small>
                                                            <small className="dir-ltr text-muted" >{item?.createDate}</small>
                                                        </div>
