...
function Index(props) {
    ...
    const [myOverseers, serMyOverseers] = useState({})
    ...
    const {listMonitoring} = useSelector(state => state.monitoring.overseerReport);
    const dispatch = useDispatch();

    function onChange(checkedValues) {
        serParams({...params})
        // serMyOverseers({getOwnSupervisors: checkedValues.target.checked})
    }
    ...
    return (
        <div className="mb-5">
            ...
            {/*    <NavLinkRouter to="/overseers/excel">*/}
            {/*        <Button size="large" type="text" icon={<AddExcelPic/>}*/}
            {/*                style={{backgroundColor: "#fff", fontSize: ".9rem"}}*/}
            {/*                className="ml-2 px-2  mb-3 mb-sm-0 text-success border-success">افزودن ناظران از فایل اکسل</Button>*/}
            {/*    </NavLinkRouter>*/}
            {/*    <NavLinkRouter to="/overseers/add">*/}
            {/*        <Button className="px-2 mb-3 mb-sm-0" size="large" style={{fontSize: ".9rem"}} icon={<AddUserPic/>}*/}
            {/*                type="primary">افزودن دستی ناظران </Button>*/}
            {/*    </NavLinkRouter>*/}
            {/*</div>*/}
            <div className="d-none d-md-block py-1">
                <span className="square-indicator ">فیلتر  جدول مانیتورینگ ناظران
</span>
                <ListFilters serParams={serParams} myOverseers={myOverseers} setIsModalVisible={setIsModalVisible}
                             InitialCDC={InitialCDC || countryDivisionCode}  back={countryDivisionCode}
                             setInitialCDC={setInitialCDC}
            ...
            <div className="py-1 mt-3">
               ...
               جدول مانیتورینگ ناظران

                        {/* <span className="text-muted mr-2 font-size-sm"> تعداد رکوردها : {listMonitoring?.length || 0} </span> */}
                    </span>
                ...
                {/*            onChange={onChange}*/}
                {/*        ><span>فقط نمایش ناظران من</span></Checkbox>*/}
                {/*    </div>*/}
                ...
                {/*            </Form.Item>*/}
                       ...
                       <div >
                           <ReportModal params={params} InitialCDC={InitialCDC} />
                       </div>
                   </div>
                {/*</div> */}
                <div>
                    <ListTable params={params} setInitialCDC={setInitialCDC} InitialCDC={InitialCDC}/>
                </div>
            </div>
