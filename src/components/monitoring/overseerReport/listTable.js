...
import {getMonitoringOverseerReport, cleanList} from '../../../redux/monitoring/overseerReport';
...
function ListTable(props) {
    ...
    const {
        ...
        listCount: listCount,
        error: ListHasError
    } = useSelector(state => state.monitoring.overseerReport);
...
// console.log(list)
    React.useEffect(() => {
        // dispatch(getMonitoringOverseerReport({countryDivisionCode}));
        if (!apiHasCalled) {
    ...
    const getData = (code = countryDivisionCode) => {
        ...
        if (code) {
            ...
            dispatch(getMonitoringOverseerReport(code, params)).then(r => setLoadingResponsive(false));
        }
    }
    // const activateUser = nationalNo => {
    //     // dispatch(overseerActivation(nationalNo, {status: 5, description: ''}))
    //     setTimeout( () => dispatch(getMonitoringOverseerReport(countryDivisionCode,{ ...params, page: currentPage})), 1500)
    // }
        ...
        <div className="mt-2">
            ...
            <Spin className="d-lg-none" spinning={loadingResponsive}>
                <TableResponsive total={listCount}
                    ...
                    //     setLoadingResponsive(true)
                    //     // dispatch(getMonitoringOverseerReport(countryDivisionCode,{ ...params, page: page})).then(r => setLoadingResponsive(false))
                    //     dispatch(getMonitoringOverseerReport(countryDivisionCode, params)).then(r => setLoadingResponsive(false))
                    //     setCurrentPage(page)
