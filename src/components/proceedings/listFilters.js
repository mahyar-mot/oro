...
function ListFilters(props) {
    ...
    const {status, proceedingType} = useSelector(state => state.basicInfo);
    ...
    React.useEffect( () => {
        ...
        if (Object.keys(querys).length){
            ...
            if (querys?.stateType) {
                // querys.stateType = ticketStateType.filter( item => item.value == querys.stateType)[0]?.name
                querys.stateType = parseInt(querys.stateType)
            }
        ...
        <div className="content px-xl-5">
            ...
            >
                ...
                <Row>
                        ...
                        <Form.Item
                            ...
                            >
                                <Select
                                    ...
                                    placeholder="  نوع صورتجلسه را انتخاب کنید"
                                    // onChange={ value => setStatusFilter(value) }
                                >
                                    {
                                        proceedingType.map((item, index) => (<Select.Option value={String(item.value)} key={index}>{item.name}</Select.Option>
                                        ))
                                    }
                                </Select>
                        </Form.Item>
                    ...
                    <Col className="px-2" xs={24} sm={12} md={12} lg={8} xl={4}>
                        <Form.Item label="جستجو" name="branchName">
                            <Input placeholder="شعبه" />
                        </Form.Item>
                    </Col>
                </Row>
                <Row>
