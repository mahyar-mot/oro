...
// import { getBranchInspectorsList, cleanBranchInspectorList } from "../../redux/headOverseers/branchInspectorsList";
import { getBranchInspectorsList, cleanBranchInspectorList } from "../../redux/branchInspectors/branchInspectorsList";
// import { getBranchInspectorBranches, cleanBranchInspectorDetail } from "../../redux/headOverseers/headOverseerDetail";
import { getBranchInspectorBranches, cleanBranchInspectorDetail } from "../../redux/branchInspectors/branchInspectorsDetail";
...
const ConfirmModal = (props) => {
    ...
    const { branchInspectorBranchesList, isLoading } = useSelector( state => state.branchInspector.retrieve )
    ...
    return(
        ...
        >
            ...
            size="small"
            loading={isLoading}
            dataSource={branchInspectorBranchesList}
...
function ListTable(props) {
    ...
    const { isLoading, listCount, branchInspectorsList } = useSelector(state => state.branchInspector.list);
    ...
    React.useEffect(() => {
        ...
        if (countryDivisionCode){
            dispatch(getBranchInspectorsList({countryDivisionCode, ...params}));
        }
    ...
    return (
        <div className="mt-2">
            ...
            <div className="d-none d-lg-block">
                <Table
                    dataSource={branchInspectorsList}
                    pagination={
                        {
                            position: ["bottomCenter"],
                            onChange: (page, pageSize) => {
                                dispatch(getBranchInspectorsList({countryDivisionCode, ...params, page, pageSize}))
                                setCurrentPage(page)
                            ...
                            <>
                                ...
                                <Button className="" type="secondary"
                                        onClick={e =>{
                                            ...
                                            dispatch(getBranchInspectorBranches(record.nationalNo, {pageSize: 20}))
                                        }}>
                                            مشاهده شعب
                                        </Button>
                            </>
            ...
            <Spin spinning={loadingResponsive}>
                <TableResponsive total={listCount}
                                 ...
                                 onChange={(page, pageSize) => {
                                     ...
                                     dispatch(getBranchInspectorsList({countryDivisionCode, ...params, page, pageSize})).then(r => setLoadingResponsive(false))
                                     setCurrentPage(page)
                                     setCurrentPageSize(pageSize)
                                 }}
                >
                    {branchInspectorsList.length ? branchInspectorsList.map((item, i) => {
